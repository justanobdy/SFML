            ...
            if (windowEvent.xclient.message_type == getAtom("XdndEnter"))
            {
                ...
                if (windowEvent.xclient.data.l[1] & 0x1)
                {
                    ...
                    if (XGetWindowProperty(m_display.get(),
                        ...
                        actualType != None)
                    {
                        const Atom* supportedAtoms = reinterpret_cast<const Atom*>(data);
...
{
    // We currently only accept uri-lists
    static const auto acceptableFileType = getAtom("text/uri-list");
    return acceptableFileType == fileType;
}
